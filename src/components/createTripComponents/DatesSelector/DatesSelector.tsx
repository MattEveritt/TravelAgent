...
import { TripModal, Card, TripText, InnerCard } from '../../travelUI';
import { RangeDatePicker } from './RangeDatePicker';
import { SingleDatePicker } from './SingleDatePicker';
import { Icon } from '@rneui/base';
import { FCLocalized } from '../../../localization/FCLocalized';
import { selectBookingDates, selectDatesValidity, setBookingDates, setDatesValidity, useAppDispatch, useAppSelector } from '../../../redux';
...
export const DatesSelector = ({
  trip, minDate = undefined, setMinDate = undefined, extraStyles = undefined, singleDate = true,
}: any) => {
  const dispatch = useAppDispatch();
  const dates = useAppSelector(selectBookingDates());
  const [modalVisible, setModalVisible] = useState(false);
  const { startDate, untilDate }: {startDate: string, untilDate: string} = dates[0];
  ...
  const dateText = singleDate
    ? startDate === 'Outbound' ? FCLocalized(startDate) : new Date(startDate).toDateString()
    : `${startDate === 'Outbound' 
  ...
  return (
    <InnerCard 
      ...
      pressDisabled={false}
      extraStyles={[styles.container, extraStyles, !datesValid && { borderColor: theme.RED_ERROR }]}
    >
      ...
      <TripModal
        ...
        headerContent={!singleDate ? () => <ModalHeaderContent dateText={dateText}/> : undefined}
